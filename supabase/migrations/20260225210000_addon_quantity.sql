-- Add quantity support to add-ons
-- Allows purchasing multiple of the same add-on per booking

-- Add max_per_order to add_ons (mirrors ticket_types pattern)
ALTER TABLE add_ons
  ADD COLUMN max_per_order integer NOT NULL DEFAULT 1;

-- Add quantity to booking add-ons junction table
ALTER TABLE event_booking_add_ons
  ADD COLUMN quantity integer NOT NULL DEFAULT 1;

-- Replace the RPC to handle add-on quantities via parallel array
...
DECLARE
  ...
  v_add_on RECORD;
  v_add_on_qty integer;
  v_add_on_idx integer;
  v_booking_id uuid;
...
BEGIN
  ...
  -- Check add-on capacities and max_per_order (using quantity from parallel array)
  IF array_length(p_add_on_ids, 1) > 0 THEN
    ...
    LOOP
      -- Find the index of this add-on in p_add_on_ids to get its quantity
      v_add_on_idx := array_position(p_add_on_ids, v_add_on.id);
      v_add_on_qty := CASE
        WHEN v_add_on_idx IS NOT NULL AND v_add_on_idx <= coalesce(array_length(p_add_on_qtys, 1), 0)
        THEN p_add_on_qtys[v_add_on_idx]
        ELSE 1
      END;

      -- Validate add-on quantity against max_per_order
      IF v_add_on_qty < 1 THEN
        RETURN jsonb_build_object('error', 'invalid_addon_quantity', 'add_on_id', v_add_on.id, 'detail', 'Quantity must be at least 1');
      END IF;
      IF v_add_on_qty > v_add_on.max_per_order THEN
        RETURN jsonb_build_object('error', 'addon_above_maximum', 'add_on_id', v_add_on.id, 'detail', 'Maximum ' || v_add_on.max_per_order || ' per order');
      END IF;

      -- Check capacity
      IF v_add_on.capacity IS NOT NULL THEN
        SELECT coalesce(sum(ba.quantity), 0) INTO v_add_on_booked
        FROM event_booking_add_ons ba
        JOIN event_bookings b ON b.id = ba.booking_id
        WHERE ba.add_on_id = v_add_on.id
          AND b.status IN ('pending', 'confirmed');

        IF v_add_on_booked + v_add_on_qty > v_add_on.capacity THEN
  ...
  -- Calculate total: ticket_price * quantity + sum(add_on_price * add_on_qty)
  v_ticket_price := v_ticket.price;
  v_total := v_ticket_price * p_quantity;

  IF array_length(p_add_on_ids, 1) > 0 THEN
    -- Sum each add-on price * its quantity
    FOR v_add_on IN
      SELECT a.id, a.price FROM add_ons a WHERE a.id = ANY(p_add_on_ids) AND a.event_id = p_event_id
    LOOP
      v_add_on_idx := array_position(p_add_on_ids, v_add_on.id);
      v_add_on_qty := CASE
        WHEN v_add_on_idx IS NOT NULL AND v_add_on_idx <= coalesce(array_length(p_add_on_qtys, 1), 0)
        ...
        ELSE 1
      END;
      v_total := v_total + (v_add_on.price * v_add_on_qty);
    END LOOP;
  END IF;
  ...
  -- Insert booking add-ons with quantities
  IF array_length(p_add_on_ids, 1) > 0 THEN
    FOR v_add_on IN
      SELECT a.id, a.price FROM add_ons a WHERE a.id = ANY(p_add_on_ids) AND a.event_id = p_event_id
    LOOP
      ...
      v_add_on_qty := CASE
        ...
        ELSE 1
      END;
      INSERT INTO event_booking_add_ons (booking_id, add_on_id, price_at_purchase, quantity)
      VALUES (v_booking_id, v_add_on.id, v_add_on.price, v_add_on_qty);
    END LOOP;
  END IF;
